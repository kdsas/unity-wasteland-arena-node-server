...
wss.on("connection", (ws) => {
  ws.on("message", (msg) => {
    ...
    const { type, username, password_hash, json, sig, secret } = data;
    ...
    // LOGIN
    if (type === "LOGIN") {
      db.get("SELECT password_hash, secret, cheat_flags FROM users WHERE username=?", [username], (e,row)=>{
        if (!row || row.password_hash !== password_hash) {
          ws.send(JSON.stringify({ type:"LOGIN_RESP", success:false }));
          return;
        }
        if (row.cheat_flags >= 3) {
          ws.send(JSON.stringify({ type:"LOGIN_RESP", success:false, message:"Banned" }));
          return;
        }
    ...
    if (type === "STATS_SAVE") {
      db.get("SELECT secret,last_stats_hash FROM users WHERE username=?", [username], (e,user)=>{
        ...
        if (user.last_stats_hash === newHash) return;

        db.get("SELECT json FROM player_stats WHERE username=?", [username], (e,row)=>{
          const oldStats = row ? JSON.parse(row.json) : {};
          const newStats = JSON.parse(json);

          if (isImpossible(oldStats,newStats)) {
